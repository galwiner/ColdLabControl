...
DEBUG=0;
initp
% p.expName='SPCM slowModeSpectroscopy probe locked 1->2 repump charecterization';
p.expName=  'SPCMslowmode repump characterization';
% p.DTPos{1} = [770,593];
% p.DTPos{2} = [387,542];
p.hasScopResults=0;
...
p.probeNDList=[1,2,3];
p.probePower=5e-11;
p.loopVals{1} = linspace(-30,15,20);
...
p.MOTReloadTime = 1e5;
p.s = sqncr;
p.s.addBlock({'setProbeDetuning','detuning',p.probeDet,'from',2,'to',3,'multiplier',64});
...
% p.s.addBlock({'setCoolingPower','duration',0,'value',p.coolingPower});
% p.s.addBlock({'setDigitalChannel','channel','coolingSwitch','duration',1e3,'value','high'});
