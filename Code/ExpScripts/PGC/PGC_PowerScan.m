...
PGCCoolingPowerVals=linspace(20,700,numSteps);
initinst
initr
p.s.getbgImg();
p.NAverage=1;
p.loopVars={'PGCCoolingPower'};
p.loopVals={PGCCoolingPowerVals};
p.(p.loopVars{1})=p.INNERLOOPVAR;
inst.DDS.setupSweepMode(1,center,span,50,1,0,p.PGCFreqRampTime)
p.PGCPowerRampTime = 1e3;
%%
p.s=sqncr();
...
p.s.addBlock({'startCoolingPowerRamp','channel','COOLVVAN','value','none','duration',p.PGCPowerRampTime,'EndPower',p.PGCCoolingPower});
% p.s.addBlock({'setAnalogChannel','channel','COOLVVAN','duration',0,'coolingPower',p.PGCCoolingPower});
...
errorbar(PGCCoolingPowerVals,averageTx,errorTx,'-ob','LineWidth',2);
hold on
errorbar(PGCCoolingPowerVals,averageTy,errorTy,'-or','LineWidth',2);
% 
ylabel('Temp [\mu K]');
% title('-10 \Gamma PGC');
legend('Tx','Ty');
subplot(2,1,2)
plot(PGCCoolingPowerVals,squeeze(r.atomNum{1}(1,:)),'-ob','LineWidth',2);
% set(gca,'FontSize',22)
% subplot(2,1,2)
