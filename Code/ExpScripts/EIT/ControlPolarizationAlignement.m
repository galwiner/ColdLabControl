...
%%
p.expName = 'ControlPolarizationAlignement';
r.OD=[];
inst.tt.setTriggerLevel(1,0.5)
p.gateNum = 10000;
p.probeLockPoint=-3;
inst.DDS.setFreq(2,probeDetToFreq(p.probeLockPoint,8));
p.BiasField=0.1;
p.probeNDList = [1,3,2];
...
chN_phot_gc{1}(chN_phot_cycles{1}>p.NAverage,:) = [];
chN_phot_gc{2}(chN_phot_cycles{2}>p.NAverage,:) = [];
chN_phot_cycles{1}(chN_phot_cycles{1}>p.NAverage) = [];
chN_phot_cycles{2}(chN_phot_cycles{2}>p.NAverage) = [];
photNum  =length(chN_phot_cycles{2})+length(chN_phot_cycles{1});
photRate = photNum/(p.gateTime/2*p.gateNum);
plot(ind,photRate,'o')
hold on
end
