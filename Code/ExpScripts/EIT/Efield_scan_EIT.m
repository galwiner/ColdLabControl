...
p.setZfield = 0;
inst.BiasE.setZField(p.setZfield);
p.delayTime = 0;
pause(p.delayTime);
...
p.hasTTresults = 1;
p.ttDumpMeasurement = 1;
p.expName = 'FastModeSpectroscopy with E Field Scan';
p.gateTime = 20;
p.gatesPerStep = 3;
...
% p.loopVals{1} = linspace(-10,10,10);
% p.loopVals{1} = 0;
% p.loopVars{1} = 'biasEx';
% p.biasEx=p.INNERLOOPVAR;
...
% p.s = sqncr;
% p.s.addBlock({p.asyncActions.setBiasE,'Ex',0});
% p.s.runStep;
p.s=sqncr();
% p.s.addBlock({p.asyncActions.setHH,'direction','y','value',p.HHYCurrent})
% p.s.addBlock({p.asyncActions.setBiasE,'Ex',p.biasEx});
...
% startInds=1:p.NAverage:p.NAverage*length(p.loopVals{1});
% stopInds=p.NAverage:p.NAverage:p.NAverage*length(p.loopVals{1});
% meansTrans=zeros(length(p.loopVals{1}),size(trans,2));
% for ind=1:length(p.loopVals{1})
% meansTrans(ind,:)=mean(trans(startInds(ind):stopInds(ind),:));
% end
% figure;
% plot(meansTrans')
% figure;imagesc(trans');title(sprintf('Bias X: %.2f V',p.loopVals{1}))
...
% profile viewer
figure;
imagesc(p.cycleTimesReltv+p.delayTime,p.smothProbeDets,trans')
ylabel('detuning [MHz]')
xlabel('Time from start [s]')
title(sprintf('setting ZEfield to %0.1f from 0',p.setZfield));
set(gca,'fontsize',12)
