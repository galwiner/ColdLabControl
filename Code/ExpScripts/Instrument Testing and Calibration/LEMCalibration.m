...
p.s=sqncr();
p.s.addBlock({'Load MOT'}); %not used for the MOT, just to get us to the same starting point
...
%% 
load('D:\Box Sync\Lab\ExpCold\Measurements\2019\01\09\090119_20.mat')
% 
...
plot(scpT,LEM,'-')
hold on
line([0.06 0.06],[0,7],'linewidth',4,'color','r')
xlabel('T [S]')
ylabel('LEM readout [V]')
title('LEM Readout with ripple end indicated')
subplot(1,2,2)
plot(p.loopVals{1},LEM(scpT==0.06),'o')
xlabel('circ coil current [A]')
ylabel('LEM readout [V]')
title('LEM voltage after ripple (60mS)')
f1=fit(p.loopVals{1}',LEM(scpT==0.06),'poly1');
hold on
fvals=linspace(20,220,200);
plot(fvals',f1(fvals),'-r','LineWidth',3)
text(100,2,sprintf('%.2f [mV/A]',1e3*f1.p1),'FontSize',22)
text(80,1.5,sprintf('zero crossing: %.2f [mV] = %.2f [A]',1e3*f1.p2,f1.p2/f1.p1),'FontSize',22)
set(gca,'fontsize',22)
% % imageViewer(r.images{1});
