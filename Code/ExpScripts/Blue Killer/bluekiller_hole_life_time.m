...
p.cameraParams{1}.ROI=[130, 120,60, 60];
initinst
initr
p.expName = 'Hole life time measurement';
inst.BiasFieldManager.I=[0.0300   -0.0850    0.0900]; %B field values to center MOT on camera
inst.DDS.setFreq(1,385);
%% 
% p.cameraParams{1}
p.flashTime=20;
...
p.runSettlingLoop=0;
p.MOTLoadTime=1e6;
p.BLUE_HOLE_Time = 2e2;
p.NAverage=3;
p.nInner = 10;
p.loopVals{1}=linspace(100,500,p.nInner);
p.loopVars{1} = 'hole_TOF_Time';
...
p.s.addBlock({p.atomicActions.pause,'duration',0.5e6});
...
p.s.addBlock({'GenPause','duration',0.5e6});
p.s.run();
