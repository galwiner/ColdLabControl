...
DEBUG=0;
initp
p.expName='DipoleTrap2StageLoadingCoolingPowerAndDetuninigScan';
p.hasScopResults=1;
...
p.loopVals{1} = linspace(-85,-50,10);
p.loopVars{1} = 'secondStageCoolingDet';
p.(p.loopVars{1}) = p.INNERLOOPVAR;

p.loopVals{2} = linspace(100,500,10);
...
for ii = 1:length(p.loopVals{1})
    if length(p.loopVals)>1
        for jj = 1:length(p.loopVals{2})
            rowNum = ceil(length(p.loopVals{2})/2);
            subplot(rowNum,2,jj)
            ax = gca;
            hold on;
            try
