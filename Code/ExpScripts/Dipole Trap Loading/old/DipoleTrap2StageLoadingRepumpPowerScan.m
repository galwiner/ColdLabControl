...
DEBUG=0;
initp
p.expName='DipoleTrap2StageLoadingRepumpPowerScan';
p.hasScopResults=1;
...
% p.secondStageCoolingDet = -70;
% p.secondStageCoolingPower = 280;
% p.secondStageRepumpPower = 0.05;
p.loopVals{1} = linspace(0.032,0.055,20);
p.loopVars{1} = 'secondStageRepumpPower';
p.DTParams.secondStageRepumpPower = p.INNERLOOPVAR;

% p.loopVals{2} = linspace(1,1000,2);
% p.loopVars{2} = 'tofTime';
% p.(p.loopVars{2}) = p.OUTERLOOPVAR;
% p.secondStageTime = 45e3;
% p.secondStageTime = 1;
p.NAverage = 5;
%scan params
p.tofTime = 700;
...
p.s.run
%%
r.scopeRes{1} = squeeze(mean(r.scopeRes{1},5));
figure;
ax = gca;
hold on;
