...
DEBUG=0;
initp
p.expName='g2 measurement, no atoms, probe alwyas on (pulse shape measurement)';
% p.DTPos{1} = [770,593];
% p.DTPos{2} = [387,542];
p.hasScopResults=0;
...
p.s.addBlock({'setDigitalChannel','channel','ControlSwitch','value','high','duration',p.gateTime});
% p.s.addBlock({'setDigitalChannel','channel','ProbeSwitch','value','high','duration',p.gateTime});
p.s.addBlock({'setDigitalChannel','channel','TTGate','value','high','duration',p.gateTime});
p.s.addBlock({'pause','duration',p.gateTime});
p.s.addBlock({'setDigitalChannel','channel','BlueDTSwitch','value','high','duration',p.gateTime});
p.s.addBlock({'setDigitalChannel','channel','PurpleDTSwitch','value','high','duration',p.gateTime});
p.s.addBlock({'pause','duration',p.gateTime});
