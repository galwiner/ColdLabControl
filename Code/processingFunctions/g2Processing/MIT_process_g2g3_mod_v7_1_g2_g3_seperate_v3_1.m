...
%%
if runs>goodruns, error('"runs" cannot be greater than "goodruns"'); end
%this section removes all runs beyong the value of the "runs" variable.
for Ic=meas_channels
    if runs<length(chN_photons_each_run{Ic})
        % Consider, in the future, to do this after the post-selection, and
        % remove alsnum_of_super_gateso the non-post-selected runs.
        chN_photons_each_run{Ic}=chN_photons_each_run{Ic}(1:runs);
        chN_gates_each_run{Ic}=chN_gates_each_run{Ic}(1:runs);
        chN_cycles_each_run{Ic}=chN_cycles_each_run{Ic}(1:runs);
        I=chN_phot_runs{Ic}>runs;
        chN_phot_runs{Ic}(I)=[];
        chN_phot_time{Ic}(I,:)=[];
        chN_phot_gc{Ic}(I,:)=[];
    end
...
%% Post-selection
%if size(ch1_photons,2)==4, chIrun=4;chIt=2;chIgate=3; else chIrun=3;chIt=1;chIgate=2; end;
%if size(ch1_photons,2)==4, chIcycle=4;chIrun=3;chIt=1;chIgate=2; else chIrun=3;chIt=1;chIgate=2; end;


%accumulate the photons per cycle across channels. (total number in the
%detector)
phot_per_cycle=0;Iremoved=[];
for Ic=meas_channels
    phot_per_cycle=phot_per_cycle+chN_photons_each_run{Ic}./chN_cycles_each_run{Ic};
end
...
if is_PostSelection_by_outgoing_rate
    ...
    if 1
        figure;all_runs=1:length(phot_per_cycle);
...
pulse_edge_time=(0:time_bin_for_pulse:Tgate/2);
runs_to_include=find(bunch_map>0);pulse_photon_rate=zeros(4,length(pulse_edge_time)-1);pulse_photon_rate_noiseless=pulse_photon_rate;   
...
% for Ic=meas_channels
        ...
        for ind_run=runs_to_include'
            ...
            ch_photons=[ch_photons;chN_phot_time{Ic}(chN_phot_runs{Ic}==ind_run,2)]; 
            % a matrix whose columns are the number of gates for all
            % included runs (for normalization)
            gates_to_include=gates_to_include+chN_gates_each_run{Ic}(ind_run);
        end        
...
%% G2
% Ic1=2;Ic2=3;
% if ~any(meas_channels==Ic1) || ~any(meas_channels==Ic2), error(654,'These channels were not measured'); end
% ch1_phot_time=chN_phot_time{Ic1};ch1_phot_gc=chN_phot_gc{Ic1};ch1_phot_runs=chN_phot_runs{Ic1};ch1_gates_each_run=chN_gates_each_run{Ic1};ch1_cycles_each_run=chN_cycles_each_run{Ic1};ch1_photons_each_run=chN_photons_each_run{Ic1};
% ch2_phot_time=chN_phot_time{Ic2};ch2_phot_gc=chN_phot_gc{Ic2};ch2_phot_runs=chN_phot_runs{Ic2};ch2_gates_each_run=chN_gates_each_run{Ic2};ch2_cycles_each_run=chN_cycles_each_run{Ic2};ch2_photons_each_run=chN_photons_each_run{Ic2};
