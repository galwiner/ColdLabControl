...
imvec=reshape(images,[arrSize(1),arrSize(2),arrSize(3)*arrSize(4)*arrSize(5)]);
x=linspace(1,arrSize(2)-1,arrSize(2))*1.65e-5*4;
y=linspace(1,arrSize(1)-1,arrSize(1))*1.65e-5*4;
x0 = 171*1.65e-5*4;
y0 = 133*1.65e-5*4;
g=9.8;
...
% end
%Fix bad points
[fp,gof,fimages]=vec2DgaussFit(x,y,imvec,bgimg,0.01155,0.008382);
% figure;
% subplot(1,2,1)
% imagesc(ROItest(:,:,1))
% hold on
...
imagesc(courrentVals,coolingPower,squeeze(tmp(1,:,:)),'AlphaData',~isnan(squeeze(tmp(1,:,:))))
title(sprintf('T_x. min temp is %0.2f [\\muK] for power = %2d and courrent = %0.2f.'...
    ,minxTmp,coolingPower(minxIndR),courrentVals(minxIndc)));
xlabel('current[A]');
...
imagesc(courrentVals,coolingPower,squeeze(tmp(2,:,:)),'AlphaData',~isnan(squeeze(tmp(2,:,:))))
title(sprintf('T_y. min temp is %0.2f [\\muK] for power = %0.0f and courrent = %2d.'...
    ,minyTmp,coolingPower(minyIndR),courrentVals(minyIndc)));
xlabel('current[A]');
ylabel('Cooling Power after DP-AOM[mW]');
set(gca,'FontSize',16)
colormap('jet')
...
[MaxAtomsInd_R,MaxAtomsInd_C]=ind2sub([length(coolingPower),length(courrentVals)],MaxAtomsInd);
figure;
imagesc(courrentVals,coolingPower,AtomNum(:,:))
colormap('jet')
title(sprintf('Atom number. max is %0.2e for power = %0.0f and current = %2d.'...
    ,maxAtoms,coolingPower(MaxAtomsInd_R),courrentVals(MaxAtomsInd_C)));
xlabel('Current[A]');
ylabel('Cooling Power after DP-AOM[mW]');
set(gca,'FontSize',16)
colorbar
...
[MaxDensityInd_R,MaxDensityInd_C]=ind2sub([length(coolingPower),length(courrentVals)],MaxAtomsInd);
figure;
imagesc(courrentVals,coolingPower,peakDensity(:,:))
colormap('jet')
title(sprintf('Peak density. max is %0.2e [cm^{-3}] for power = %0.0f and current = %2d.'...
    ,maxDensity,coolingPower(MaxDensityInd_R),courrentVals(MaxDensityInd_C)));
xlabel('Current[A]');
ylabel('Cooling Power after DP-AOM[mW]');
set(gca,'FontSize',16)
colorbar
