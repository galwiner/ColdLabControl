    ...
    methods
        function obj=Block(inputSeq,name)
            ...
            obj.asyncActionInputMap = {{'setProbeDetuning','detuning','from','to','multiplier'},...
                ...
                {'configDoubleBPulse','firstB','secondB','direction','scannedVal','t1','t2'}};
        ...
        function atomizeAll(obj,seq)
            if nargin==1 %if no seq is input, set default at obj.b;
                seq=obj.b;
            end
            if isempty(seq) %if seq is empty, warn and exit
                warning('Cannot atomize empty Block');
                return
            end
            if strcmpi('Block',class(seq)) %if seq is a Block, take it's cell array (b)
                seq=seq.b{1};
            end
            for ii=1:size(seq,1) %loop over seq (action nember)
                temp=obj.atomizer(seq{ii}); %atomize current action.
                if ~exist('newseq','var') %if newseq doesn't exist, set it as temp, else concatenate
                    newseq=temp;
                else
                    newseq={newseq{:},temp{:}}';
                end
            end
            obj.b=newseq;  %set the action array as the atomized action array (newseq).
            if ~any(obj.isAtomic(newseq)) %if there are still non-atomic actions, go again (recursively)
                obj.atomizeAll(newseq);
            end
        end
        function atomizedAction=atomizer(obj,action)
            global p;
            if obj.isAtomic(action) %if action is atomic, return it and exit
                atomizedAction={action};
                return
            end
            if size(action,2)>1 %if parameters ware input, define them as 'params' and 'values'
                dat={action{2:end}};
                if ~mod(size(dat,2),2)==0 %verify that there all parameters are in name-value pair
                    error('Action/Block parameters need to be key-value pairs');
                end
                params=dat(1:2:end); %param names
                values=dat(2:2:end); %param values        
            end
            switch action{1} %switch through all the different compound actions, and form there atomized action 
                case 'Load MOT'
                    atomizedAction={
            ...
            if exist('params','var') %passing parameter values on
                for ii=1:size(atomizedAction,1) %loop over all action in the atomized action list
                    found=zeros(1,size(params,2));
                    for jj=1:size(atomizedAction{ii},2) %loop over all parts of the atomic action atomizedAction{ii}
                        for kk=1:size(params,2) %loop over all params
                            bool=strcmp(params{kk},atomizedAction{ii}{jj}); %compare params{kk} to atomizedAction{ii}{jj}
                            if bool %if match found, flag it (not in use as of 17/03/20)
                                found(kk)=1;
                            end
                            if bool %if parameter found, pass it's value to the atomic action.
