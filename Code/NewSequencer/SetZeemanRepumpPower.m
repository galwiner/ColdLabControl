function [state,maxp,minp] = SetZeemanRepumpPower(pwr,ndlist)
global inst
global p
...
if strcmpi(pwr,'max')
    [~,maxp,minp]=calculateAtten(ndlist,'zeemanRepump');
    pwr=maxp*0.99; %in [mW]
...
p.s = sqncr;
p.s.addBlock({'setZeemanRepumpPower','duration',0,'value',pwr,'ND',ndlist});
p.s.runStep;
state = 1;
end
