classdef sqncr < handle & matlab.mixin.CustomDisplay
    ...
    methods
        ...
        function populatedSeq=populateVars(obj,fullSeq,innerLoopVal,outerLoopVal)
            global p
            for ii = 1:size(fullSeq,1)% loop over the lines of fullSeq and varify all entries are 1x1 cell array.
            ...
            for ii=1:size(fullSeq,1) % loop over the lines of fullSeq
        ...
        function asyncRes=executeAsync(obj,seq)
            ...
            tBlock=Block(); %call tmp Block, to use isAsync
            if isempty(seq)
                asyncRes=NaN;
                return
            end
            ...
            for ii=1:size(seq,1) %loop over all actions (lines) in seq
                % a switch containing all the different async actions
                switch seq{ii}{1}
                    case 'setDDSfreq'
        ...
        function [syncRes]=executeSync(obj,seq)
            ...
            tBlock=Block(); %call a temporary Block, to use Block methods
            block=Block(seq);% make seq into a Block object.
            block.setTimeline(0); %reset time line 
            seq=block.b;% get a cell array of all the actions in seq
            ...
            for ii=1:size(seq,1) %loop over all actoins
                %list of spasial cases, for which we set channel, value or
                %duration explicitly.
                switch seq{ii}{1}
                    case 'pause' %pause command is not exequted, it adds pause in timeLine
                        continue
                    case 'forEnd'
                    ...
                    case 'setRepumpDetuning'
                        error('setRepumpDetuning is dissabled as of 16/03/2020')
                        seq{ii}{end+1}='channel';
                        seq{ii}{end+1} = 'RPMPVCO';
                    case 'setImagingDetuning'
                        error('setImagingDetuning is dissabled as of 16/03/2020')
                        seq{ii}{end+1}='channel';
                    ...
                    case 'setImagingPower'
                        error('setImagingPower is dissabled as of 16/03/2020')
                        seq{ii}{end+1}='channel';
                        seq{ii}{end+1} = 'ImagingVVAN';
                    case 'set480ControlPower'
                        error('set480ControlPower is an async action as of 16/03/2020')
                        seq{ii}{end+1}='channel';
                        seq{ii}{end+1} = 'ImagingVVAN';
                    case 'setBlueDTPower'
                        seq{ii}{end+1}='channel';
                    ...
                    case 'set776ControlPower'
                        error('set776ControlPower is dissabled as of 16/03/2020')
                        seq{ii}{end+1}='channel';
                        seq{ii}{end+1} = 'ImagingVVAN';
                    case 'setProbePower'
                        seq{ii}{end+1}='channel';
                ...
                %get channelName.
                if ~strcmpi(seq{ii}{1},'GenPause')%GenPause dosen't have channelName
                    chanIdx=find(strcmpi(seq{ii},'channel'));
                    if isempty(chanIdx)%if no chanIdx, report and error
                        error('Error: No channel name in action %s',seq{ii}{1});
                    end
                    channelName=seq{ii}{chanIdx+1};
                    % if channelName==coolvvan, set replace 'coolingPower' with 'value' 
                ...
                if isempty(widthInd) %if no widthInd, report and error
                    error('Error: No duration in action %s',seq{ii}{1});
                end
                width=seq{ii}{widthInd+1};
                %get start time.
                startTimeInd = find(strcmpi(seq{ii},'start time'));
                if isempty(startTimeInd)%if no startTimeInd, report and error
                    error('Error: No startTime in action %s',seq{ii}{1});
                end
                startTime=seq{ii}{startTimeInd+1};
                %get value
                if ~strcmpi(seq{ii}{1},'GenPause')%GenPause dosen't have value
                    ValueInd = find(strcmpi(seq{ii},'value'));
                    if isempty(ValueInd)%if no ValueInd, report and error
                        error('Error: No value in action %s',seq{ii}{1});
                    end
                    Value=seq{ii}{ValueInd+1};
                end
                %get Inverted, i.e is pulse inverted (alwase ann appart
                %from duration)
                ...
                %switch the different options for atomic actions, and add
                %each to FPGAseq
                switch seq{ii}{1}
                    case 'GenPause'
                        if ii==size(seq,1)
                ...
                %if this is the first time, make new FPGASeq, else,
                %concatenate
            ...
            %find the start time of final action. This is the delay
            %duration needed in Matlab for the FPGA to finish
...
%                 error('pause can''t be last action. Use GenPause instade')
            end
            if exist('FPGASeq','var')%if FPGASeq exist exacute the FPGA sequence.
                seqUpload(FPGASeq,p.DEBUG); 
                %changed on 16/03/2020 by L.D. If this brakes somthing,
                %uncomment (also lines 1053+1054)
