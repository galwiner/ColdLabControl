...
%                         if ~p.loopingRun || p.firstLoopingStep % L.D
%                         commented this on 05/02/20. this shoulent be hare
%                         and firstLoopingStep is not a field of p, would
...
%                     pauseTime=seq{end}{delayInd+1}*1e-6+0.1+width*1e-6; 
        ...
        function run(obj)
            ...
            if isfield(p,'biasField') %to help with backwards compatibility with a different spelling.
                p.BiasField = p.biasField;
            end
            if ~p.loopingRun
                I = inst.BiasFieldManager.I;
                B = inst.BiasFieldManager.I .*  inst.BiasFieldManager.conversionFactors;
                B0 = p.B0;
            ...
            if isempty(p.loopVals) && isempty(p.loopVars)
                if isfield(p,'runSettlingLoop') && p.runSettlingLoop
                    ...
                    for pp = 1:p.settlingStepN
                        obj.runStep();
                    end
                end
                if p.hasPicturesResults&&~p.pfLiveMode
                    [imWidth,imHight] = inst.cameras('pixelfly').getImSize;
                   r.images{1} = zeros(imHight,imWidth,p.picsPerStep,1,1,p.NAverage);
                end
                stepCtr=1;
                p.cycleTimes = ""; %reset cycle times to delete runSettlingLoop cycles
                p.ttStartTime = [];%reset ttStartTime to delete runSettlingLoop cycles
                p.TTGateStartTimes = [];%reset TTGateStartTimes to delete runSettlingLoop cycles
                for ii = 1:p.NAverage
                    run_timer=tic;
                    if p.NAverage == 1
...
%                             tmp=inst.cameras('pixelflyPlane').getImages(picsPerStep);
                            ...
                            [w,h] = inst.cameras('pixelfly').getImSize;
                             r.images{1}(:,:,:,1,1,ii) = nan([h,w,picsPerStep]);
...
%                         fprintf('pausing 0.0s for tt dump readout\n');
                            pause(0.3);
                             tt_tmr2 = tic;
                ...
                if p.absImagePostproc
                    absImagePostproc()
                end
                p.cycleTimesReltv = [];
                p.cycleTimesReltv(1) = 0;%relative cycle times, in s, with respect to first cycle
                zeroChar = char(p.cycleTimes(1));
                for ii = 2:length(p.cycleTimes)
                    tmpChar = char(p.cycleTimes(ii));
                    p.cycleTimesReltv(ii) = 60*60*(str2double(tmpChar(1:2))-str2double(zeroChar(1:2)))+...%hour diff
                        60*(str2double(tmpChar(4:5))-str2double(zeroChar(4:5)))+...%minute diff
                        str2double(tmpChar(7:8))-str2double(zeroChar(7:8))+...%seconds diff
                        1e-3*(str2double(tmpChar(10:12))-str2double(zeroChar(10:12)));%ms diff
                end
                if ~p.loopingRun %prevents data saving if we are looping (e.g. in fast mode spect.)
                    customsave;
                else
                    warning('p.loopingRun==1, customsave did not run');
                end
                if p.notificationOn          
...
%                     disp('1103')
            ...
            if p.hasPicturesResults&&~p.pfLiveMode
                    [imWidth,imHight] = inst.cameras('pixelfly').getImSize;
                   r.images{1} = zeros(imHight,imWidth,p.picsPerStep,NOuter,NInner,p.NAverage);
                end
            stepCtr=1;
           p.cycleTimes = ""; %reset cycle times to delete runSettlingLoop cycles
           p.ttStartTime = [];%reset cttStartTime to delete runSettlingLoop cycles
           p.TTGateStartTimes = [];%reset TTGateStartTimes to delete runSettlingLoop cycles
            for ii=1:NOuter
                if ii~=1
                   if isfield(p,'M2scanMode_Analog')&&p.M2scanMode_Analog==1
                       resetSolsTissResonatorSlow(p.loopVals{1}(end),p.loopVals{1}(1))
                   end
                end
                for jj =1:NInner
                    if isfield(p,'M2scanMode_Analog')&&p.M2scanMode_Analog==1
                            ...
                            if picsPerStep~=1
                                if ~p.pfLiveMode
                                    try
                                        ...
                                        [w,h] = inst.cameras('pixelfly').getImSize;
                                        r.images{1}(:,:,:,ii,jj,kk) = nan([h,w,picsPerStep]);
%                                         error(err.identifier,'can''t access pixelflyPlane. Check p.pfLiveMode\n%s',err.message);
                                ...
                                if ~p.idsLiveMode
                                    try
                                        tmp=inst.cameras('ids').getBufferImages;
                                        r.images{2}(:,:,1:size(tmp,3),ii,jj,kk)=tmp;
                                    catch err
                                        error(err.identifier,'can''t access IDS. Check p.IDSLiveMode\n%s',err.message);
                                    end
...
%                             disp('1221')
                        ...
                        if isfield(p,'M2scanMode')&&p.M2scanMode==1
                            MOTVal = mean(r.scopeRes{1}(:,2,ii,jj,kk));
                            r.TisaphFreqs(jj,ii) = getWLMFreq(8);
%                             if MOTVal<0.90*p.MOTRefVal
%                                 p.MOTLoadTime = 0.5e6;
%                                 p.endPauseTime = 40e3;
%                                 %change scope timebase
%                                 inst.scopes{1}.setTimebase(p.MOTLoadTime*1e-6);
%                                 inst.scopes{1}.setDelay(-p.MOTLoadTime*1e-6/2);
%                                 pause(0.05);
%                             else
%                                 p.MOTLoadTime = 5e4;
%                                 p.endPauseTime = 1e3;
%                                 %change scope timebase
%                                 inst.scopes{1}.setTimebase(0.05);
%                                 inst.scopes{1}.setDelay(-0.05/2);
%                                 pause(0.03);
%                             end
                        end
                    end
