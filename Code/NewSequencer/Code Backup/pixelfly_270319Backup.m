classdef pixelfly < handle
    ...
    properties (Access=private)
        ...
        binning_free_scale=8.5575e-06; %m/pix. as of 23/10/18
        Plane2TopRatio =3.047; %Measured at 17/01/19.
    end
    ...
    methods
        ...
        function obj=pixelfly(varargin)
            if nargin == 0
                error('Pixelfly object must have a name! The options are: plane and top')
            end
            if nargin == 1
                obj.name = varargin{1};
            end
            if strcmpi(obj.name,'top')
               obj.binning_free_scale = 4.0994e-06;%m/pix. calibrated using test target. 
            end
...
%             end
            planeID = 0;
            TopID = 1;
            if strcmpi(obj.name,'plane')
                obj.ID = planeID;
            elseif strcmpi(obj.name,'top')
                obj.ID = TopID;
            else
                error('Pixelfly name must be ''plane'' or ''top''')
            end
            try
            obj.handle = videoinput(driverName,obj.ID, 'USB 2.0');
            catch err
                disp(err.identifier);
