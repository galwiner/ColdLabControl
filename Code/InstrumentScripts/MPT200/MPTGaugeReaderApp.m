classdef MPTGaugeReaderApp < handle
    %
    % Usage:
    %     app.MPTGaugeReaderApp
    ...
    properties (Constant)
        AppPathBase = {'/MPTGaugeReader/code'};
        AppClass = 'MPTGaugeReaderApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function obj = MPTGaugeReaderApp()
        ...
        function startApp(obj)
            ...
            mlock;
            MPTGaugeReaderApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'MPTGaugeReader')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong MPTGaugeReader
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'MPTGaugeReader').getString, ...
            ...
            if nargout(@MPTGaugeReader) == 0
                eval('MPTGaugeReader');
            else
                obj.AppHandle = eval('MPTGaugeReader');
            end
