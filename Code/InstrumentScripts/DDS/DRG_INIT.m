...
% obj.s = serial('COM10','BaudRate',9600,'DataBits',8);
% fopen(obj.s);
fsys=1000;
...
DRG_LIMITS_DOWN=round((2^32)*(DOWN_FREQUENCY)/(fsys));
   ...
   fsys_timing=1000e6;
   Mi=dec2binvec(dfP*2^32/fsys_timing,32)/1e6;
   Md=dec2binvec(dfN*2^32/fsys_timing,32)/1e6;
%    m=dfN*2^32/fsys
   i= (dtP*fsys/4);
   fsys_timing1=1000e6;
   PtP=dec2binvec(dtP*fsys_timing1/4,16);
   PtM=dec2binvec(dtN*fsys_timing1/4,16);
